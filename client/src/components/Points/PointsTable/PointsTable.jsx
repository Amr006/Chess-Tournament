...
const PointsTable = ({ data }) => {
  ...
  const createData = (...args) => {
    ...
    args
      ...
      .map((round, i) => (row[`round${i + 1}`] = round));
    return row;
  };
  ...
  data.map((player) =>
    rows.push(
      createData(
        player.Name,
        ...Array.from({ length: player.Matches.length }, (_, i) => {
          ...
          <TableRow>
            {columns.map((column, i) => (
              ...
              >
                <Typography
                  variant="h5"
                  className={`${i > 0 && i < columns.length - 1 && "tac"}`}
                >
                  {column.label}
                </Typography>
              </TableCell>
            ...
            return (
              <TableRow
                className={`${styles.table_cells} ${
                  row.name === username && styles.active
                }`}
                key={row.name}
              >
                <TableCell
                  key={i}
                  className={`flex-start ${styles.player}`}
                  component="th"
                  scope="row"
                >
                  <Box
                    className={`flex-center ${i + 1 === 1 && styles.first} ${
                      styles.player_photo
                    }`}
                  >
                      ...
                      >
                        {
                          <Button
                            onClick={() =>
                              window.open(
                                data[i].Matches[x-2].gameLink,
                                "_blank"
                              )
                            }
                            className={`${styles.round_point}`}
                          >
                            {row[key]}
                          </Button>
                        }
